...
class SearchMailingListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url_to_test = reverse('archives:user_list_subscriptions',
    ...
    def test_show_all_mailinglist_for_subscription(self, mock):
        ...
        response = self.client.post(self.url_to_test, {},
                                    HTTP_X_REQUESTED_WITH='XMLHttpRequest')

        self.assertContains(response, self.all_lists[0]['listname'],
                            status_code=200)
        self.assertContains(response, self.all_lists[1]['listname'],
                            status_code=200)
        self.client.logout()
    ...
    def test_show_searched_mailinglist_for_subscription(self, mock):
        ...
        response = self.client.post(self.url_to_test, data,
                                    HTTP_X_REQUESTED_WITH='XMLHttpRequest')

        self.assertContains(response, data['listname'], status_code=200)
        self.assertNotContains(response, self.all_lists[1]['listname'],
                               status_code=200)
        self.client.logout()
    ...
    def test_show_message_mailinglist_not_found_for_subscription(self, mock):
        ...
        self.assertNotContains(response, self.all_lists[0]['listname'],
                               status_code=200)
        self.assertNotContains(response, self.all_lists[1]['listname'],
                               status_code=200)
        self.client.logout()
